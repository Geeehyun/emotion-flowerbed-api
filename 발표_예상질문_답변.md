...
**현재 프로젝트에는 Claude Code Hooks가 설정되어 있지 않습니다.**

하지만 일반적으로 Git 커밋 시 민감정보 검증을 위해 사용하는 패턴들을 소개합니다:
...
jwt\.secret-key\s*=\s*[^\s]+
```

**4. Redis 비밀번호**
```regex
redis\.password\s*=\s*[^\s]+
```

### Pre-commit Hook 구현 예시
...
anthropic:
  api:
    key: ${SPRING_ANTHROPIC_API_KEY}  # ✅ 환경 변수 사용

openai:
  api:
    key: ${SPRING_OPENAI_API_KEY:your-openai-api-key}  # ⚠️ 기본값 노출

jwt:
  secret-key: ${SPRING_JWT_SECRET_KEY:your-secret-key-min-256-bits...}  # ⚠️ 기본값 노출
```

**권장 사항**:
- `.env` 파일 사용 (Git 제외)
- 프로덕션 환경: AWS Secrets Manager 또는 환경 변수
- 기본값 제거: `${SPRING_JWT_SECRET_KEY}` (기본값 없이)
